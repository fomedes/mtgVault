...
export const boardActionSchema = z.discriminatedUnion("type", [
  z.object({ type: z.literal("MOVE_ON_BATTLEFIELD"), instanceId, x: coord, y: coord }),
  ...
  z.object({ type: z.literal("SET_COUNTER"), instanceId, key: z.string().min(1).max(24), value: z.number().int() }),
  z.object({ type: z.literal("ADJUST_COUNTER"), instanceId, key: z.string().min(1).max(24), delta: z.number().int() }),
  z.object({ type: z.literal("MOVE_CARD"), instanceId, target: moveTargetSchema }),
  ...
  z.object({ type: z.literal("SCRY_REORDER"), order: z.array(instanceId).min(1).max(50) }),
  ...
  z.object({ type: z.literal("SET_LIFE"), seat: z.number().int().min(0).max(3), value: z.number().int().min(-999).max(999) }),
  z.object({ type: z.literal("ADJUST_LIFE"), seat: z.number().int().min(0).max(3), delta: z.number().int().min(-999).max(999) }),
  z.object({ type: z.literal("SET_ACTIVE_SEAT"), seat: z.number().int().min(0).max(3).nullable() }),
...
export const setDeckSchema = z.object({
  ...
  source: z.discriminatedUnion("kind", [
    z.object({ kind: z.literal("deck"), deckId: z.string().regex(/^[0-9a-fA-F]{24}$/) }),
    z.object({ kind: z.literal("decklist"), text: z.string().min(1).max(20_000) }),
  ]),
});

export const sessionOnlySchema = z.object({ sessionId });

export const inviteSchema = z.object({ sessionId, friendUid: z.string().min(1).max(128) });
